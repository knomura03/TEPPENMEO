...
describe("job_runs マイグレーション判定", () => {
  it("エラーなしなら適用済み", () => {
    const result = resolveJobRunsSchemaStatus(null);
    expect(result.status).toBe("ok");
    expect(result.issue).toBeNull();
  });

  it("テーブル未作成を検知する", () => {
    const result = resolveJobRunsSchemaStatus({
      code: "42P01",
      message: "relation \"job_runs\" does not exist",
    });
    expect(result.status).toBe("missing");
    expect(result.issue).toBe("table_missing");
  });
