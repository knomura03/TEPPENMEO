...
| ダッシュボード | `/app` | テナント | 状態把握 | 主要導線の確認 | app-dashboard-enhanced / app-dashboard-polish |
...
| 口コミ・コメント受信箱 | `/app/reviews` | テナント | 口コミ対応 | 横断一覧/フィルタ/検索/返信（Google/FB/IGコメント） | app-reviews-inbox-with-social-comments / app-reviews-inbox-real-mode-guidance / app-reviews-polish |
| 店舗一覧 | `/app/locations` | テナント | 店舗管理 | 作成/一覧/検索 | app-locations-ui-primitives |
| 店舗詳細 | `/app/locations/[id]` | テナント | 連携/口コミ/投稿 | 連携サービスの接続/店舗選択/口コミ取り込み/投稿（テンプレ対応）/履歴 | app-location-detail-simple / app-location-detail-post-template / app-location-detail-polish |
...
| リリース準備 | `/admin/release` | システム管理 | リリース前チェック | 環境/マイグレーション/権限/Runbook導線 | admin-release-branding / admin-release-templates / admin-release-env-templates / admin-release-evidence-link / admin-release-acceptance / admin-release-polish |
